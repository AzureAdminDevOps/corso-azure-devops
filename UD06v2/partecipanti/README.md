...
## Convenzione dei terminali

Quando un blocco è marcato:

```text
bash
```

deve essere eseguito nella shell **Ubuntu/WSL2 Bash** usata per il laboratorio, salvo indicazione esplicita diversa.

Quando il materiale richiede il Portale Azure, viene scritto:

```text
Portale Azure
```

Gli script non banali sono forniti nella directory:

```text
partecipanti/script/
├── select-vm-target.sh
└── prepare-appservice.sh
```

Il LAB indica sempre:
